//!  Virtual IMU implementation code / Código de implementación del IMU virtual
/*!
  En este archivo se contruyen las interacciones entre el Imu virtual y ROS. Representa al sensor propiamente y debe lanzarse con imu_data_node.cpp para la configuración
*/
 ...
 *	@date 10/12/2019
 *  @author Gustavo Sosa
 *	@brief Código de implementación del IMU virtual
 */
  ...
  void TurtleImuSim::spawnAndConfigureVisualizationTurtle(const turtlesim::Pose & initial_pose) {
    if(isVisualizationRequested() && !isVisualizationTurtleAvailable()) {
      ...
      */
      ...
      // Log message.
      ROS_INFO("Relative position measurement (IMU) visualized by '%s' with a green pen.", visualization_turtle_name_.c_str());
    }
  }
